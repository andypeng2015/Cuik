...
struct Ctx;
static TB_Node* isel_va_start(struct Ctx* ctx, TB_Function* f, TB_Node* n);
...
static TB_Symbol* gimme_float_neg_zero(TB_Module* mod, TB_DataType dt) {
    if (dt.type == TB_TAG_F32) {
        uint32_t imm[4];
        FOR_N(i, 0, 4) { imm[i] = 0x80000000; }

        TB_Global* g = tb__small_data_intern(mod, sizeof(imm), imm);
        return &g->super;
    } else {
        uint64_t imm[2];
...
static TB_Symbol* gimme_float_sym(TB_Module* mod, TB_Node* n) {
    size_t elem_size = 0;
    void* elem = 0;
    ...
    TB_Global* g = tb__small_data_intern(mod, vec_size, x);
    return &g->super;
}
